...
def csv_to_wikidata(
    ...
    write_mappings_to_csv(node_ids, fname, input_file, input_file_rel, edge_type_ids) #written by counttrucola authors
...
def csv_to_tkg_data(
    ...
    if 'yago' in fname or 'icews14' in fname or 'icews18' in fname or 'wikiold' in fname or 'gdelt' or 'monkey' or 'pig' or 'crisis' in fname:
        ...
        input_file = None
        input_file_rel = None
    write_mappings_to_csv(node_ids, fname, input_file, input_file_rel) #written by counttrucola authors
...
def write_mappings_to_csv(node_ids:dict, fname:str, input_file:str="None", input_file_rel:str="None", rel_ids=None):
    """
    write the node mappings to a csv file
    Parameters:
        node_ids: dictionary of node names mapped to integer node ids
        fname: str, name of the input file
        input_file: str, name of the input file for the original node names
        input_file_rel: str, name of the input file for the original relation names
        rel_ids: optonal dictionary of relation names mapped to integer relation ids (only used for wikidata, smallpedia)"""

    output_file = osp.join(osp.dirname(fname), 'node_mapping.csv')
    output_file_rel = osp.join(osp.dirname(fname), 'rel_mapping.csv')
    string_dict = None
    if input_file !=None:
        string_dict = {}
        input_dir = osp.join(osp.dirname(fname), input_file)
        with open(input_dir, 'r', encoding='utf-8') as file:
            for line in file:
                parts = line.strip().split('\t')
                if len(parts) < 2:
                    continue
                if 'wikidata' in fname or 'smallpedia' in fname:
                    string, original_id = parts[1], parts[0]
                else:
                    string, original_id = parts[0], int(parts[1])
                # if 'wiki' in fname or 'smallpedia' in fname: # we have an additional third column in the 'entity2id.txt' file with the original name
                #     try:
                #         string = parts[2]
                #     except:
                #         string= string
    ...
    data = [None]*len(node_ids)
    for index, node in enumerate(node_ids):
        if string_dict != None:
            data[index] = [ node, node_ids.get(node), string_dict.get(node)]
        else:
            data[index] = [ node, node_ids.get(node)]

    # Write to a CSV file
    with open(output_file, 'w', newline='', encoding='utf-8') as file:
        writer = csv.writer(file, delimiter=';')
        writer.writerow(['original_id', 'tgb_id', 'string'])  # Write header
        writer.writerows(data)

    # string_dict = {}
    data = []
    if input_file_rel !=None:
        input_dir = osp.join(osp.dirname(fname), input_file_rel)
        with open(input_dir, 'r', encoding='utf-8') as file:
            for line in file:
                parts = line.strip().split('\t')
                if len(parts) < 2:
                ...
                if 'wikidata' in fname or 'smallpedia' in fname:
                    ...
                    string, original_id = parts[0], int(parts[1])
                if 'gdelt' in fname: # we have an additional third column in the 'relation2id.txt' file with the original name
                    try:
                        string = parts[2]
                    except:
                        string= string
                # string_dict[original_id] = string
                if rel_ids != None:
                    data.append([original_id, rel_ids.get(original_id), string])
                else:
                    data.append([original_id, string])
    else:
        if rel_ids != None:
            for index, rel in enumerate(rel_ids):
                data.append([index, rel_ids.get(rel), ''])

    # Write to a CSV file
    with open(output_file_rel, 'w', newline='', encoding='utf-8') as file:
        writer = csv.writer(file, delimiter=';')
        if rel_ids != None:
            writer.writerow(['original_id', 'tgb_id', 'string'])
        else:
            writer.writerow(['original_id=tgb_id', 'string'])  # Write header
        writer.writerows(data)

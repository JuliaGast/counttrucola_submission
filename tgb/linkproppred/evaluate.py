...
class Evaluator(object):
    ...
    def _eval_hits_and_mrr(self, y_pred_pos, y_pred_neg, type_info, k_value):
        ...
        Parameters:
            ...
            k_value: the desired 'k' value for calculating metric@k. int or list of int
        ...
        if type_info == 'torch':
            ...
            if type(k_value) == list:
                ...
                for k in k_value:
                    ...
                    results_dict[f'hits@{k}'] = hitsK_list.mean()
            else:
                hitsK_list = (ranking_list <= k_value).to(torch.float)
                results_dict[f'hits@{k_value}'] = hitsK_list.mean()
            mrr_list = 1./ranking_list.to(torch.float)
            results_dict['mrr'] = mrr_list.mean()
            return results_dict
            ...
            # if optimistic_rank == pessimistic_rank:
            #     a =1
            # else:
            #     a =2
            #     # print('same')

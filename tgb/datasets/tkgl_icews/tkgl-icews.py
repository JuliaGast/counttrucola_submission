    ...
    with open(fname, 'r', encoding='ISO-8859-1') as f:
        ...
        for row in reader: 
            ...
            if ("None" in date or "None" in head or "None" in tail or "None" in relation_type):
                ...
                num_lines += 1
                if (ts in out_dict):
                    if (head, tail, relation_type, relation_string) in out_dict[ts]:
                        out_dict[ts][(head, tail, relation_type, relation_string)] += 1
                    else:
                        out_dict[ts][(head, tail, relation_type, relation_string)] = 1
                else:
                    out_dict[ts] = {}
        ...
        for date in out_dict:
            for edge in out_dict[date]:
                ...
                if relation_type not in edge_type_dict:
                    edge_type_dict[relation_type] = (max_edge_type_id, relation_string)
                    max_edge_type_id += 1
...
def writeEdgeTypeMapping(edge_type_dict, outname):
    ...
    with open(outname, 'w', encoding="utf-8", newline='') as f:
        ...
        for key in edge_type_dict:
            if type(edge_type_dict[key]) == int:
                writer.writerow([key, edge_type_dict[key]])
                
            else:
                writer.writerow([key, edge_type_dict[key][0], edge_type_dict[key][1]])
...
def main():
    ...
    writeEdgeTypeMapping(node_dict, "tkgl-icews_nodemapping.csv")

...
def writeNodeTypeMapping(edge_type_dict, outname):
    r"""
    write the edge type mapping to a file
    """
    with open(outname, 'w', encoding='ISO-8859-1', newline='') as f:
        writer = csv.writer(f, delimiter =',')
...
def main():
    ...
    for file in glob.glob("*.csv"):
        if ("edgelist" in file) or ('nodemapping' in file) or ('edgemapping' in file):
            continue
    ...
    writeNodeTypeMapping(node_dict, "tkgl-polecat_nodemapping.csv")
